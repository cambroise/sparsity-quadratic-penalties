...
The departure from zero for \mytexttt{glmnet} illustrates its relative imprecision, which is responsible for the difference in the set of selected markers.
%
\begin{figure}
  \centering
  \includegraphics[width=5cm]{../figures/gradient}
  \caption{Distribution of the absolute value of the derivatives with respect to the non-zero coefficients of the parameter vectors returned by \mytexttt{glmnet} (left) and \mytexttt{quadrupen} (right)}.
  \label{fig:gradient}
\end{figure}
