...
In the following we argue that \mytexttt{glmnet} precision is highly affected by the threshold controling the stopping condition. A first hint of the difference of precision of the two methods can be seen in the  absolute  value of the gradient component for all non zero entries of the parameter vector (see Figure  \ref{fig:gradient}).


\begin{figure}[htbp]
  \centering
  \includegraphics[width=5cm]{../figures/gradient}
  \caption{Distribution of the abolute value of the gradient for all non zero entries of the parameter vector according the optimization method (\mytexttt{quadrupen} on the left and \mytexttt{glmnet} on the right)}.
  \label{fig:gradient}
\end{figure}
