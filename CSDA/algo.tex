...
of the solution by solving a series of small linear systems, whose sizes are
...
variables, $\supp$, indexing the coefficients $\bfbeta_{\!\supp}$ currently
identified as being non-zero. 
It is started from a sparse initial guess, say $\supp=\emptyset$ ($\bfbeta=0$),
...
\item \label{item:algo:step2} the second step updates $\bfbeta_{\!\supp}$
  ...
  Section~\ref{sec:gammaperturb}, where $\clD_{\bfgamma}$ is a convex polytope
...
%  \DontPrintSemicolon
  ...
  Determine the active set: $\supp \leftarrow \{j:\beta_{j} > 0 \} $\\
  Pick a worst admissible $\bfgamma$, that is,  $\displaystyle\bfgamma \in 
  \argmax_{\bfg \in \clD_{\bfgamma}} \norm{\bfbeta - \bfg}^2$
  \BlankLine 
  ...
  $\bfbeta_{\!\supp} \leftarrow 
   \left(\mathbf{X}_{\centerdot\supp}^\intercal
      \mathbf{X}_{\centerdot\supp} + \lambda \mathbf{I}_{|\supp|}\right)^{-1}
  ...
  \If(\tcp*[f]{if $\bfgamma_{\!\supp}$ is not worst-case})
   {$\displaystyle\norm{\bfbeta_{\!\supp} - \bfgamma_{\!\supp}}^2 < \max_{\bfg \in \clD_{\bfgamma}} \norm{\bfbeta_{\!\supp}-\bfg_{\!\supp}}^2$}
   {%
  ...
  \If(\tcp*[f]{if $\widetilde\bfgamma_{\!\supp}$ is worst-case\ldots})
   {$\displaystyle\norm{\widetilde\bfbeta_{\!\supp} - \widetilde\bfgamma_{\!\supp}}^2 = \max_{\bfg \in \clD_{\bfgamma}} \norm{\widetilde\bfbeta_{\!\supp}-\bfg_{\!\supp}}^2$}
     {%
%        $\bfbeta_{\!\supp}^\mathrm{old} \leftarrow \bfbeta_{\!\supp}$ \\
  ...
  $\displaystyle g_j \leftarrow \min_{\bfgamma\in\clD_{\bfgamma}} \left|
    \mathbf{x}_j^\intercal(\mathbf{X}_{\centerdot\supp}\bfbeta_{\!\supp} - \mathbf{y})  + \lambda (\beta_j - \gamma_j) \right|
  \enspace j=1,\ldots,p$
  \tcp*[f]{worst-case gradient} \\
  \eIf{$\exists\, j \in\supp:\beta_j = 0 \ \text{and}\ g_j = 0$}{
  ...
  \end{small} 
\caption{Worst-Case Quadratic Penalty Active Set Algorithm}
...
\iflong
  ...
  $\supp^c$). However this $\bfgamma$ fails to belong to $\clD_{\bfgamma}$ 
  ...
  \begin{proposition}\label{prop:monitoring}
    ...
    \begin{equation*}
      ...
      \enspace.
    \end{equation*}
    See proof in Appendix \ref{sec:proof:prop:monitoring}.
  \end{proposition}
  ...
  \begin{figure}
    \centering 
    \xylabellarge{../figures/monitoring_bounds}{\# of iterations}{Optimality gap}{$n=50$, $p=200$}
    \caption{Monitoring convergence: true optimality gap (solid black) versus
    our generic upper bound (dashed blue) and Fenchel's duality gap for elastic
    net (dotted red) computed at each iteration of
  ...
  We see that the two optimality gaps can be used to assess convergence, though
  Fenchel's duality gap is more accurate for the rougher solutions. 
  Note however that both gaps are fairly coarse until a very accurate
...
%   ineffective as stopping rules when fast and rough solutions are sought.
  ...
  computing the optimality gap in Appendix~\ref{sec:proof:prop:monitoring}, and
